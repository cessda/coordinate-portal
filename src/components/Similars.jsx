...
export class Similars extends React.Component {
  ...
  render() {
    ...
    if (this.state.loading == true) {
      ...
      let htm = [];
      let datacount = 1;
      /* Value has to be 1 in order to skip the current dataset in the elasticsearch query,
       which is always the first result with the highest score */

      // check whether one similar dataset is found
      if (this.state.ssimilarData.length !== 0 && (this.state.ssimilarData[2] !== undefined)) {
        let nrToShow = 3;

        for (var int = 0; int < nrToShow; int++) {
          let tit = 'test';
          if (this.state.ssimilarData[datacount].fields['dc.description.all'] !== undefined) {
            if (this.state.ssimilarData[datacount].fields['dc.description.all'][0] !== '' ||
                this.state.ssimilarData[datacount].fields['dc.description.all'][0] !== null) {
              tit = this.state.ssimilarData[datacount].fields['dc.description.all'][0];
            }
          } else {
            if (this.state.ssimilarData[datacount].fields['dc.title.all'][0] !== '' ||
                this.state.ssimilarData[datacount].fields['dc.title.all'][0] !== null) {
          ...
          if (this.state.ssimilarData[datacount]._id != this.state.currentEsid &&
              this.state.ssimilarData.length > datacount) {
          ...
          else if (this.state.ssimilarData[datacount]._id == this.state.currentEsid &&
                   this.state.ssimilarData.length > nrToShow) {
            htm.push(React.DOM.div({
              className: 'simItem',
              key: 'simItem' + datacount,
              style: divStyle
