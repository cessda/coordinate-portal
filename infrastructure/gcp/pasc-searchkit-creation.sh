...
sed "s/DEPLOYMENTNAME/$CLIENT-$PRODUCT-$MODULE-$ENVIRONMENT/g; s/NAMESPACE/$CLIENT-$PRODUCT-$ENVIRONMENT/g; s/PVCNAME/$CLIENT-$PRODUCT-$ENVIRONMENT-pvc/g; s/ESURL/$CLIENT-$PRODUCT-es-$ENVIRONMENT/g; s/IMAGENAME/$PRODUCT-$MODULE:$ENVIRONMENT/g" ../k8s/template-pasc-$MODULE-deployment.yaml > ../k8s/$CLIENT-$PRODUCT-$MODULE-$ENVIRONMENT-deployment.yaml
