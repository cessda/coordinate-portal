...
* FIXED ~~Elasticsearch CMMStudy JSON does not contain a list of languages for which other metadata is available. This is required for the language buttons displayed for each result on the search page. Therefore only the current language appears in that list. (See `/src/reducers/search.js` line `79`)~~
